...
const emptyTask = (durationValue = 15): TaskDraft => ({
  title: "", keyPoints: "", focusTags: [], customFocus: "", isHighPriority: false, durationValue,
});
...
export default function AddClass() {
  ...
  const [savedTags, setSavedTags] = useState({ teachers: [] as string[], styles: ["Hiphop", "House", "Freestyle"] });
  ...
  return <AppShell active="/add-class"><div className="page add-class-page">
    <Header eyebrow={`第 ${step}/2 步 · ${step === 1 ? "课程复盘" : "练习任务"}`} title={step === 1 ? "快速记下这节课。" : "安排下一步练习。"} action={step === 2 ? <button className="round-button" onClick={() => setStep(1)} aria-label="返回课程复盘">←</button> : undefined} />
    ...
    {step === 1 ? <form onSubmit={continueToTasks} className="real-form">
      <section className="form-panel compact-class-panel"><div className="form-step"><span>01</span><div><h2>课程信息</h2><p>必要信息集中填写，之后查找更快</p></div></div>
        <div className="input-grid core-class-grid"><label className="date-field"><span>日期 *</span><input type="date" value={classInfo.date} onChange={e => updateClass("date", e.target.value)} /></label><label><span>课程主题 *</span><input placeholder="例如：Bounce & Arms" value={classInfo.classTheme} onChange={e => updateClass("classTheme", e.target.value)} /></label></div>
        <div className="input-grid compact-tag-grid">
        <div className="repeatable-tag-field"><span>老师 *</span>{savedTags.teachers.length > 0 && <div className="tag-picker compact">{savedTags.teachers.map(tag => <button type="button" className={classInfo.teacher === tag ? "selected" : ""} key={tag} onClick={() => chooseTag("teacher",tag)}>{tag}</button>)}</div>}<input aria-label="自定义老师" placeholder={savedTags.teachers.length ? "输入新老师，按回车确认" : "输入老师名称"} value={classInfo.teacher && !savedTags.teachers.includes(classInfo.teacher) ? classInfo.teacher : ""} onChange={e => updateClass("teacher",e.target.value)} onKeyDown={e => acceptTagInput(e,"teacher")} /></div>
        ...
        {(showDifficulty || showBodyStatus) && <details className="optional-details"><summary>可选：状态和难度</summary><div className="optional-details-body">
        {showDifficulty && <div className="optional-block"><span>课程难度 <small>选填</small></span><div className="tag-picker compact">{[["Beginner zero","零基础"], ["Beginner","初级"], ["Improving","提高"], ["Custom","自定义"]].map(([value,label]) => <button type="button" className={classInfo.difficulty === value ? "selected" : ""} key={value} onClick={() => updateClass("difficulty",value)}>{label}</button>)}</div>{classInfo.difficulty === "Custom" && <input aria-label="自定义难度" placeholder="填写难度名称" value={classInfo.customDifficulty} onChange={e => updateClass("customDifficulty",e.target.value)} />}</div>}
        {showBodyStatus && <div className="optional-block"><span>上课状态 <small>选填</small></span><div className="feeling-picker">{([['Tired','疲惫'],['Okay','还行'],['Great','特别好']] as const).map(([value,label]) => <button type="button" className={classInfo.classCondition === value ? "selected" : ""} key={value} onClick={() => updateClass("classCondition",classInfo.classCondition === value ? "" : value)}>{value === "Tired" ? "◔" : value === "Okay" ? "●" : "✦"} {label}</button>)}</div></div>}
        </div></details>}
      </section>
      <section className="form-panel compact-notes-panel"><div className="form-step"><span>02</span><div><h2>课堂笔记 <small>选填</small></h2><p>只写最有用的一两句就好</p></div></div>
        <label><span>今天学会了什么？</span><textarea rows={2} placeholder="动作、感觉、老师提醒，都可以" value={classInfo.whatILearned} onChange={e => updateClass("whatILearned", e.target.value)} /></label>
        <label><span>下次练习线索</span><textarea rows={2} placeholder="不用判断是否消化，只记下想回头看的点" value={classInfo.notDigested} onChange={e => updateClass("notDigested", e.target.value)} /></label>
      </section>
      <section className="form-panel compact-video-panel"><div className="form-step"><span>03</span><div><h2>视频引用 <small>选填</small></h2><p>只记录复盘视频的位置，不会上传视频</p></div></div>
      ...
      {tasks.map((task, index) => <section className="form-panel task-editor compact-task-editor" key={index}><div className="task-editor-head"><div className="form-step"><span>{String(index + 1).padStart(2,"0")}</span><div><h2>练习任务</h2><p>来自这节课，不需要重复选择来源</p></div></div>{tasks.length > 1 && <button type="button" onClick={() => setTasks(current => current.filter((_, i) => i !== index))}>删除</button>}</div>
        ...
        <label><span>任务要点 <small>选填</small></span><textarea rows={2} placeholder="只在有帮助时补充提示" value={task.keyPoints} onChange={e => updateTask(index,{keyPoints:e.target.value})} /></label>
        ...
        <div className="duration-editor"><span>练习倒计时 <small>默认 {defaultDuration} 分钟，可修改</small></span><div className="quick-duration task-duration-picks">{DEFAULT_PRACTICE_DURATION_OPTIONS.map(value => <button type="button" className={task.durationValue === value ? "selected" : ""} key={value} onClick={() => updateTask(index,{durationValue:value})}>{value} 分钟</button>)}</div><label><span>自定义分钟数</span><input aria-label={`任务 ${index + 1} 时长`} type="number" min="1" max="999" value={task.durationValue} onChange={e => updateTask(index,{durationValue:Math.max(1,Number(e.target.value))})} /></label></div>
