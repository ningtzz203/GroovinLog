...
export default function AddPracticeLog() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const [review, logs] = await Promise.all([
          found.classReviewId ? repository.findClassReview(found.classReviewId) : Promise.resolve(undefined),
        ...
        const existingLog = logId ? logs.find(log => log.id === logId && log.taskId === found.id) ?? null : null;
        setEditingLog(existingLog);
        if (existingLog) {
          setForm({
            date:existingLog.date,
            durationValue:existingLog.durationMinutes ?? existingLog.durationValue,
            practiceContent:existingLog.practiceContent,
            progressScore:existingLog.progressScore,
            nextFocus:existingLog.nextFocus,
          });
          return;
        }
  ...
  async function submit(event: FormEvent) {
    ...
    const logId = editingLog?.id ?? (pendingLogId || crypto.randomUUID());
    ...
    const log: PracticeLog = {
      ...
      songsCount:null,
      practiceContent:form.practiceContent.trim(), progressScore:form.progressScore, nextFocus:form.nextFocus.trim(), createdAt:editingLog?.createdAt ?? new Date().toISOString(),
    };
    setSaving(true);
    setError("");
    try {
      const repository = await getDataRepository();
      if (editingLog) {
        await repository.updatePracticeLog(editingLog.id, {
          date:log.date,
          durationUnit:log.durationUnit,
          durationValue:log.durationValue,
          durationMinutes:log.durationMinutes,
          songsCount:log.songsCount,
  ...
  async function deleteLog() {
    if (!editingLog || saving || deleting) return;
    setDeleting(true);
    setError("");
    try {
      const repository = await getDataRepository();
      await repository.deletePracticeLog(editingLog.id);
      router.push(task?.classReviewId ? `/classes/${task.classReviewId}?practiceDeleted=1` : "/practice?practiceDeleted=1");
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "练习记录删除失败，请稍后重试。");
      setDeleting(false);
    }
  }
  ...
  return <AppShell active="/practice"><div className="page practice-log-page">
    <Header eyebrow="练习记录" title={editingLog ? "编辑练习记录" : "今天练得怎么样？"} action={<Link href="/practice" className="round-button" aria-label="关闭练习记录">×</Link>} />
    ...
    <form className="real-form" onSubmit={submit}>
      ...
      <button className="primary-button enabled" type="submit" disabled={saving || deleting}>{saving ? "保存中…" : editingLog ? "保存修改" : "保存练习记录"} <Icon name="arrow" /></button>
    </form>
    {editingLog && <section className="danger-zone"><strong>删除这条练习记录？</strong><p>这只会删除本条练习记录，不会删除练习任务。此操作不能撤销。</p>{confirmingDelete ? <div className="record-actions"><button type="button" disabled={saving || deleting} onClick={() => setConfirmingDelete(false)}>Cancel</button><button type="button" className="danger-button" disabled={saving || deleting} onClick={deleteLog}>{deleting ? "删除中…" : "Confirm Delete"}</button></div> : <button type="button" className="danger-button" disabled={saving || deleting} onClick={() => setConfirmingDelete(true)}>Delete</button>}</section>}
  </div></AppShell>;
}
