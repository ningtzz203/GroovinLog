...
export default function Practice() {
  ...
  const loadPracticeData = useCallback(async () => {
    try {
      ...
      const [classReviews, standaloneTasks, nextLogs, preferences] = await Promise.all([
        repository.readClassReviews(),
        repository.readStandaloneTasks(),
        repository.readPracticeLogs(),
        repository.readPreferences(),
      ]);
      const classTasks = classReviews.flatMap(review => review.tasks.map(task => ({ ...task, source: `${review.teacher} · ${review.danceStyle} · ${review.date}` })));
      const standalone = standaloneTasks.map(task => ({ ...task, source: "独立练习" }));
      ...
      setTaskError(caught instanceof Error ? caught.message : "练习队列读取失败，请稍后重试。");
    } finally {
      setLoaded(true);
    }
  }, []);
  ...
  async function changeStatus(taskId: string, status: "digested" | "practicing") {
    if (actionPending) return;
    setActionPending(`status:${taskId}`);
    setTaskError("");
    try {
      const repository = await getDataRepository();
      await repository.updatePracticeTask(taskId,{status});
      await loadPracticeData();
      setSaveMessage(status === "digested" ? "任务已标记为已消化。" : "任务已回到进行中。");
    } catch (caught) {
      setTaskError(caught instanceof Error ? caught.message : "任务状态更新失败，请稍后重试。");
    } finally {
      setActionPending("");
    }
  }
  ...
  async function setPreferenceDuration(value: number) {
    if (actionPending) return;
    const durationValue = Math.max(1, Math.round(value));
    setActionPending("preferences");
    setTaskError("");
    try {
      const repository = await getDataRepository();
      const preferences = await repository.savePreferences({ defaultPracticeDurationMinutes:durationValue });
      setDefaultDuration(preferences.defaultPracticeDurationMinutes);
      setDraft(current => current.title || current.keyPoints || current.focusTags.length || current.customFocus ? current : { ...current, durationValue:preferences.defaultPracticeDurationMinutes });
      setSaveMessage(`默认练习时长已设为 ${preferences.defaultPracticeDurationMinutes} 分钟。`);
    } catch (caught) {
      setTaskError(caught instanceof Error ? caught.message : "默认练习时长保存失败，请稍后重试。");
    } finally {
      setActionPending("");
    }
  }
  ...
  function toggleEditFocus(focus: string) {
    ...
    setActionPending(`edit:${editingTaskId}`);
    setTaskError("");
    try {
      const repository = await getDataRepository();
  ...
  async function createStandaloneTask(event: FormEvent) {
    ...
    setActionPending("standalone");
    setTaskError("");
    try {
      const repository = await getDataRepository();
      await repository.saveStandaloneTask(task);
      await loadPracticeData();
      ...
      setTaskError(caught instanceof Error ? caught.message : "独立任务保存失败，请稍后重试。");
    } finally {
      setActionPending("");
    }
  }
  ...
  return <AppShell active="/practice"><div className="page practice-page"><Header eyebrow="练习队列" title="今天想练什么？" />
    <section className="practice-preference-card"><div><small>默认练习时长</small><strong>{defaultDuration} 分钟</strong><p>创建课程任务或独立任务时会自动带入，单个任务仍可修改。</p></div><div className="quick-duration preference-duration-picks">{DEFAULT_PRACTICE_DURATION_OPTIONS.map(value => <button type="button" disabled={Boolean(actionPending)} className={defaultDuration === value ? "selected" : ""} key={value} onClick={() => void setPreferenceDuration(value)}>{value}</button>)}</div><label><span>自定义</span><input aria-label="默认练习时长" type="number" min="1" max="999" value={defaultDuration} onChange={e => void setPreferenceDuration(Number(e.target.value))} /></label></section>
    ...
    {showComposer && <form className="standalone-composer real-form subtle-composer" onSubmit={createStandaloneTask}><div className="composer-heading"><Icon name="practice" /><div><strong>添加独立任务</strong><p>Free Practice Task · 不关联课程复盘。</p></div></div>
      ...
      {taskError && <p className="form-error" role="alert">{taskError}</p>}<button className="primary-button enabled" type="submit" disabled={Boolean(actionPending)}>{actionPending === "standalone" ? "保存中…" : "保存独立任务"} <Icon name="arrow" /></button>
    </form>}
    ...
    <section><SectionTitle>练习任务</SectionTitle><div className="practice-list">
      {!loaded && <EmptyState icon="practice" title="正在加载练习队列" text="请稍候。" />}
      {loaded && savedTasks.length === 0 && <><EmptyState icon="practice" title="练习队列还是空的" text="先从课程复盘创建任务；如果只是自主练习，也可以添加独立任务。" /><Link className="secondary-button empty-state-action" href="/add-class"><Icon name="plus" /> 添加课程复盘</Link></>}
